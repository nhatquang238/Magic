  ...
  return function() {
    ...
    if ((_ref = this.get(identifier)) != null) {
      _ref.find('img').each(function() {
        var expansion, img, rarity, _ref1;
        img = $(this);
        _ref1 = /^(.*\S)\s+[(](.+)[)]$/.exec(img.attr('alt')).slice(1), expansion = _ref1[0], rarity = _ref1[1];
...
common_attrs = {
  ...
  types: function(data) {
    var subtypes, text, type, types, _i, _len, _ref, _ref1;
    if (!(text = this.text('Types'))) {
      return;
    }
  ...
  rulings: function() {
    var d, date, el, m, ruling, rulings, y, _i, _len, _ref, _ref1, _ref2;
    rulings = [];
    _ref = this.$('.cardDetails').find('tr.post');
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      el = _ref[_i];
      _ref1 = this.$(el).children(), date = _ref1[0], ruling = _ref1[1];
      _ref2 = this.text(date).split('/'), m = _ref2[0], d = _ref2[1], y = _ref2[2];
      if (m.length === 1) {
        m = '0' + m;
      }
      if (d.length === 1) {
...
gid_specific_attrs = {
  flavor_text: function(data) {
    ...
    if (!(flavor = this.get('Flavor Text'))) {
      return;
    }
...
list_view_attrs = {
  ...
  types: function(data) {
    var loyalty, power, regex, subtypes, text, toughness, type, types, _i, _len, _ref, _ref1;
    if (!(text = this.text('.typeLine'))) {
      return;
    }
    regex = /^([^\u2014]+?)(?:\s+\u2014\s+(.+?))?(?:\s+[(](?:([^\/]+(?:[{][^}]+[}])?)\s*\/\s*([^\/]+(?:[{][^}]+[}])?)|(\d+))[)])?$/;
...
exports.set = function(body, callback) {
  var $, card, cards, ctx, data, el, error, expansion, fn, href, id, key, page, pages, param, params, rarity, underlined, value, _i, _len, _ref, _ref1, _ref2, _ref3, _ref4;
  $ = cheerio.load(body);
  ctx = {
    $: $,
    text: text_content
  ...
  page = underlined.length ? +ctx.text(underlined) : 1;
  if (+((_ref = $('#aspnetForm').attr('action').match(/page=(\d+)/)) != null ? _ref[1] : void 0) + 1 === page) {
    ...
    for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
      ...
      for (key in list_view_attrs) {
        ...
        _ref2 = /multiverseid=(\d+)/.exec(href), param = _ref2[0], id = _ref2[1];
        params = '?' + param;
        card.gatherer_url = gatherer_base_card_url + params;
        card.image_url = gatherer_image_handler + params + '&type=card';
        _ref3 = get_versions('.setVersions').call(ctx)[id], expansion = _ref3.expansion, rarity = _ref3.rarity;
